...
import taskRemindersFakerFactory from "./taskRemindersFakerFactory";
...
const TaskRemindersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = taskRemindersFakerFactory(props.user,fakerCount,taskIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("taskReminders").create(elem));
...
export default connect(mapState, mapDispatch)(TaskRemindersFakerDialogComponent);
