...
import tasksFakerFactory from "./tasksFakerFactory";
...
const TasksFakerDialogComponent = (props) => {
...
const [assigneeIds, setassigneeIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("projects")
                        ...
                        .then((data) => {
                        setprojectIds(data.data);
                        });
client.service("users")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setassigneeIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = tasksFakerFactory(props.user,fakerCount,projectIds,assigneeIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("tasks").create(elem));
...
export default connect(mapState, mapDispatch)(TasksFakerDialogComponent);
