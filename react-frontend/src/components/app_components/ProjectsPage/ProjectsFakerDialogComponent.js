...
import projectsFakerFactory from "./projectsFakerFactory";
...
const ProjectsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = projectsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("projects").create(elem));
...
export default connect(mapState, mapDispatch)(ProjectsFakerDialogComponent);
