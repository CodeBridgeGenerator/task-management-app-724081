...
const ProjectsCreateDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [members], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,description: _entity?.description,status: _entity?.status,members: _entity?.members?.map((e) => e.value),dueDate: _entity?.dueDate,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Projects updated successfully" });
    ...
    const membersOptions = members.map((elem) => ({ name: elem, value: elem }));
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Dropdown id="members" value={_entity?.members?._id} optionLabel="name" optionValue="value" options={membersOptions} onChange={(e) => setValByKey("members", {_id : e.value})}   itemTemplate={membersTemplate} panelFooterTemplate={membersPanelFooterTemplate} selectedItemTemplate={membersSelectedItemTemplate}  />
            </span>
            <small className="p-error">
