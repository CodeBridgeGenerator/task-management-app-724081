...
const TaskCommentsCreateDialogComponent = (props) => {
...
const [author, setAuthor] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [task,author], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.content)) {
                error["content"] = `Comment field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            task: _entity?.task?._id,author: _entity?.author?._id,content: _entity?.content,
        ...
        const result = await client.service("taskComments").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Task Comments updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Task Comments" });
        }
        setLoading(false);
    };
...
const authorOptions = author.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create Task Comments" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="taskComments-create-dialog-component">
...
export default connect(mapState, mapDispatch)(TaskCommentsCreateDialogComponent);
